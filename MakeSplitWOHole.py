...
writefile=r"C:\Users\eivinhug\NTNU\PhD\AbaqusModels\SplitDisk\Split_Friciton_study_Wo_Hole" #Work directory
...
for i in range(0,len(Friction)):
    ...
    MRad=MeshRadN[-1]
    MSides=MeshSidesN[-1]
    MCoarse=100 #MeshCoarseN[-1]
    MEdges=40
    Fric=Friction[i]
    n=n+1

    #--------- Initiate ----------
    modelname="SplitDiskWOHole_%s"%(n)
    jobname="Job_SplitDiskWOHole_%s"%(n)
    ...
    XYPlotName="XY_Plot_SplitWOHole_LE22_%s"%(n)    
    XYPlotName2="XY_Plot_SplitWOHole_LE11_%s"%(n) 
    #-----------------------------
    ...
    session.viewports['Viewport: 1'].setColor(colorMapping=cmap)
    session.viewports['Viewport: 1'].disableMultipleColors()
    p = mdb.models['SplitDiskWOHole_1'].parts['Part-1']
    c = p.cells
    cells = c.getSequenceFromMask(mask=('[#d ]', ), )
    p.Set(cells=cells, name='Set_Sect_8_3')
    p = mdb.models['SplitDiskWOHole_1'].parts['Part-1']
    c = p.cells
    cells = c.getSequenceFromMask(mask=('[#12 ]', ), )
    p.Set(cells=cells, name='Set_Sect_Hoop')    
    ...
    faces1 = f1.getSequenceFromMask(mask=('[#1011088 ]', ), )
    ...
    faces1 = f1.getSequenceFromMask(mask=('[#884202 ]', ), )
    ...
    a = mdb.models[modelname].rootAssembly
    f1 = a.instances['Part-1-1'].faces
    faces1 = f1.getSequenceFromMask(mask=('[#220850 ]', ), )
    ...
    #a = mdb.models['SplitDiskWOHole_1'].rootAssembly
    #s1 = a.instances['Part-1-1'].faces
    side1Faces1 = s1.getSequenceFromMask(mask=('[#400000 ]', ), )
    #a.Surface(side1Faces=side1Faces1, name='Set_Surf_Pipe')    
    ...
    pickedEdges = e.getSequenceFromMask(mask=('[#22428905 #128 ]', ), )
    p.seedEdgeByNumber(edges=pickedEdges, number=MCoarse, constraint=FIXED)
    p = mdb.models[modelname].parts['Part-1']
    
    e = p.edges
    edges = e.getSequenceFromMask(mask=('[#22428905 #128 ]', ), )
    p.Set(edges=edges, name='Set_Seed_Coarse')
    
    #--------- Edges ------------
 
    pickedEdges = e.getSequenceFromMask(mask=('[#1809602a #284 ]', ), )
    p.seedEdgeByNumber(edges=pickedEdges, number=MEdges, constraint=FIXED)
    p = mdb.models[modelname].parts['Part-1']
    
    e = p.edges
    edges = e.getSequenceFromMask(mask=('[#1809602a #284 ]', ), )
    p.Set(edges=edges, name='Set_Seed_Edges')    
